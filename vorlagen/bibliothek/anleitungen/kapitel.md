Erschließe den genannten Beitrag.

## Erst lesen

1. `medien/<slug>/beitrag.md` — was steht schon da? Handgeschriebene
   Beschreibung, vorhandene Kapitel, Schlagworte, Dauer.
2. `medien/<slug>/transcript.json` — die Quelle. Ein Feld `segments`, jedes mit
   `start`, `end`, `text`.
...
Drei Blöcke in `medien/<slug>/beitrag.md`, mit `Edit`, ausschließlich zwischen
den Markern.
