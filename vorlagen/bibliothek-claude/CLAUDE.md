...
`bezuege`, `anhaenge` — und in `themen/` zusätzlich `fundstellen`.
...
| Befehl | Was er tut |
|---|---|
| `/kapitel <slug>` | Kapitel, Gesamt- und Kapitelzusammenfassung für einen Beitrag |
| `/kapitel-alle` | dasselbe für alle Beiträge ohne Kapitel, in einer Sitzung |
| `/bezuege <slug>` | verwandte Stellen in allen anderen Beiträgen finden |
| `/themen` | Themenseiten mit Synonymen und Fundstellen anlegen und pflegen |
| `/glossar` | Fachbegriffe mit korrekter Schreibweise sammeln |

Die Regeln jedes Befehls stehen in `.claude/commands/<name>.md` — dort
genauer als hier.
