...
function templateDir(): string {
  return path.join(process.cwd(), "vorlagen", "bibliothek");
}
...
export async function installInstructions(): Promise<InstallResult> {
  ...
  for (const file of files) {
    const target = path.join(paths.library, ...file.split("/"));
    try {
...
/**
 * Liegen die Anleitungen schon in der Bibliothek? Für die Anzeige.
 *
 * Geprüft wird `anleitungen/REGELN.md` und nicht `.claude/CLAUDE.md`: die
 * Anleitungen sind die Wahrheit und gelten für jedes Werkzeug, der
 * Claude-Ordner ist nur eine Abkürzung.
 */
export async function instructionsInstalled(): Promise<boolean> {
  try {
    await fs.access(path.join(paths.library, "anleitungen", "REGELN.md"));
    return true;
  } catch {
    return false;
  }
